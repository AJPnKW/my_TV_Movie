...
def write_reports(repo_root: Path, rows: list[QaRow]) -> Path:
    ...
    (out / "playback_qa.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8", newline="\n")
    with (out / "playback_qa.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    (out / "playback_qa.html").write_text(f"<!doctype html><html><head><meta charset='utf-8'><title>Playback QA</title><style>body{{font-family:Segoe UI;background:#07101f;color:#eef5ff}}td,th{{border:1px solid #26395e;padding:5px}}table{{border-collapse:collapse;width:100%}}</style></head><body><h1>Playback QA</h1><table><tr><th>Status</th><th>File</th><th>Video</th><th>Audio</th><th>Reason</th><th>Path</th></tr>{html_rows}</table></body></html>", encoding="utf-8", newline="\n")
    return out
