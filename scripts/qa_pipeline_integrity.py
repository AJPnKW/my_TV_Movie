...
def main() -> int:
    ...
    try:
        ...
        _check("inputs_editor_publishes_and_syncs_generated_artifacts", all(term in inputs_editor_server_text for term in ("/api/publish-inputs", "/api/publish-status", "_editor_publish_status", "_wait_for_generated_artifacts", "_stash_generated_artifacts_if_needed", "_generated_artifact_changes_since", "_has_runtime_artifact_update", "_ensure_publishable_git_state", "diff-filter=U", "Git conflict state could not be checked", "git diff --cached failed", "qa_pipeline_integrity.py")), "inputs editor publish path must expose publish status, wait for generated runtime artifact changes, block unresolved Git conflicts and failed conflict scans, fast-forward local checkout, and validate reconciliation", checks, logf)
