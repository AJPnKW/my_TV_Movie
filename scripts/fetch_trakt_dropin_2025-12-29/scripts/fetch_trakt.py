...
def main() -> int:
    ...
    # Snapshot for change detection (avoid rewriting data.json when nothing changed)
    _before_snapshot = copy.deepcopy(existing)
except Exception as e:
        logging.error("[fetch_trakt] %s", e)
        return 4
    ...
    logging.info(
        "[fetch_trakt] DONE looked_up=%s not_found=%s updated_shows=%s updated_movies=%s already_shows=%s already_movies=%s",
