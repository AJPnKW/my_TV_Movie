...
$Log = Join-Path $LogDir "apply_$Stamp.log.txt"
...
Set-Content -LiteralPath $Log -Value "Applying latest safe cleanup plan..." -Encoding UTF8
...
Invoke-NativeLogged "Apply safe cleanup plan" {
    Invoke-Python312 @("tools\media_renamer\media_cleanup_pipeline.py", "apply", "--repo-root", $RepoRoot, "--media-root", $MediaRoot)
}

Write-LogLine "Cleanup apply completed. Log: $Log"
Write-Host "Cleanup apply completed. Log: $Log"
