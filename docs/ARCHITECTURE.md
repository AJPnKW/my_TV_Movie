...
- Pages deploys only the explicit runtime JSON set above plus canonical `data/inputs.json`, `data/discover_registry.json`, `data/provider_registry.json`, and `data/watch_state_queue.json`; helper/report JSON under `data/` must not be deployed by wildcard.
