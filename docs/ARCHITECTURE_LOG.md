...
- Removed the generated split runtime catalog design from production: `data/catalog_index.json`, `data/calendar.json`, `data/catalog_detail/*.json`, and `scripts/build_split_runtime.py` were deleted.
- Repointed active runtime loading, calendar derivation, show/movie detail lookup, Inputs Editor publish synchronization, GitHub build/deploy workflows, pipeline integrity checks, runtime catalog validation, and media library helpers to the single generated runtime catalog `data/data.json`.
