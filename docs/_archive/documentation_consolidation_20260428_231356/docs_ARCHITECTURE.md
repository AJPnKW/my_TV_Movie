...
- Local launcher: root `run_local_servers.bat` starts/reuses the static app server on `127.0.0.1:8000` and the Inputs Editor API server on `127.0.0.1:8787`; `run_server.bat`, `tools/run_local_servers.bat`, and `tools/start_inputs_editor.cmd` are compatibility delegators only.
- Legacy redirects:
...
- Card action icon order is locked:
  ...
  - Current rendered icons are `🍿`, `⌚`, `🎫`, `💕`, and compact percent text such as `76%`.
  - Legacy play, bookmark, single-heart, ruler, and star treatments are deprecated outside historical docs.
- Shared cards are rendered through `web/js/card_renderer.js` and the shared action row from `web/js/action_bar.js`.
- Poster/still media never sits behind actions. The action row is below the media area, and cards keep a title/meta text row outside image overlays.
- Card availability badges are not rendered over media. The popcorn action communicates availability state: green available, orange not yet released, red unavailable.
- Watch-state identity is context-specific:
  - episode watched status: `watched_status:episode:<show_id>:<season>:<episode>`
  - movie watched status: `watched_status:movie:<tmdb_id>`
  - show watched status: `watched_status:show:<tmdb_id>`
  - `watch_list` and `favourite` follow the same item context, with `favourite` remaining local-only.
- Page shells use `assets/custom/the_boys_hub_logo2.png` as the compact sticky header logo. Nav remains keyboard and D-pad reachable with icon-first buttons and accessible labels.
- Dashboard section headers use the shared sticky section pattern so current week, watchlist, upcoming, and recommendations replace each other without overlapping cards.
- Dashboard and calendar overflow uses the shared `+X more` expansion pattern. Silent fixed three-item truncation is forbidden.
...
- Runtime poster targets are `171x257`; runtime episode still targets are `256x180` after cropping 10 percent from each side of the source; runtime backdrop targets are at most `780px` wide. Runtime folders must not retain 4K/original-sized images.
