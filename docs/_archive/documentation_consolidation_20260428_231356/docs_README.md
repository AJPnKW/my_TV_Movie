...
- Current action icon order is popcorn, watch, ticket, double-heart, compact percent rating.
- Shared cards keep action rows below media, remove availability badges from card images, and use the shared `+X more` overflow behavior on dashboard/calendar.
- Runtime asset targets are posters `171x257`, episode stills `256x180` after a 10 percent side crop, and backdrops no wider than `780px`.
- Historical docs may mention old bookmark, single-heart, star, play, or ruler treatments; current docs must use `docs/UI_COMPONENTS.md` and `docs/DOCUMENTATION_STANDARD.md`.
