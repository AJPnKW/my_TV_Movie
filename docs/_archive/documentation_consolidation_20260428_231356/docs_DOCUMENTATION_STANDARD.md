...
|---|---|---|---|---|---|
| action icons | `web/js/action_bar.js` | `docs/UI_COMPONENTS.md`, `docs/ARCHITECTURE.md` | `web/js/ui_contract_fix.js`, `web/css/ui_contract_fix.css` | legacy play, bookmark, single-heart, ruler, and star treatments | `powershell -ExecutionPolicy Bypass -File scripts/validate_runtime.ps1` |
...
Movies and episodes render `🍿`, `⌚`, `🎫`, `💕`, and compact percent text such as `76%`.

Shows and seasons render `⌚`, `🎫`, `💕`, and compact percent text.

The third icon is the ticket icon. The fourth icon is the double-heart icon. Compact ratings do not include a star icon.
